...
bool RedisManager::Start(const RedisConf &redis_conf) {
    ...
    for (int j = 0; j < 2; ++j) {
        for (int i = 0; i < redis_num_; ++i) {
            threads_
                .create_thread(bind(&RedisManager::SendToRedis, this, &context_managers_[j][i]));
        }
    }
...
bool RedisManager::ExcuteCommandInternal(const RedisServer &server,
    ...
    *reply =
        reinterpret_cast<redisReply *>(redisCommand(context->execute_context, command.c_str()));
    if (*reply == NULL) {
        LOG(WARNING) << "Got NULL reply for " << command << ": "
