...
static int
get_agent_config_cb(void *arg, int argc, char **argv, char **col_name) {
...
TaskManager::TaskManager()
    ...
    control_server_
        .set_accept_callback(boost::bind(&TaskManager::OnAccept, this, _1));
    control_server_.set_read_callback(boost::bind(&TaskManager::OnReadMessage,
                                                  this,
                                                  _1,
                                                  _2));
}
...
shared_ptr<TaskManager::metadata_t>
TaskManager::GetMetaByInfohash(const string &infohash) {
    shared_ptr<metadata_t> meta;
    scoped_lock lock(metas_lock_);
    for (metadata_list_t::iterator it = metas_.begin(); it != metas_.end();
         ++it) {
...
void TaskManager::AddResumeTask() {
    ...
    scoped_lock lock(tasks_lock_);
    for (readdir_r(dir, &entry, &entptr); entptr != NULL;
         readdir_r(dir, &entry, &entptr)) {
        ...
        shared_ptr<Task>
            task = Task::CreateFromResumeFile(file_name, statbuf.st_size);
        if (!task) {
...
bool TaskManager::GetAgentOptionFromDb(message::AgentOptions *options) {
    ...
    string sql =
        "select bind_port, upload_limit, connection_limit from agent_config limit 1";
    for (int i = 0; i < 3; ++i) {
    ...
    if (options->upload_limit() == -1) {
        ...
        std::stringstream s;
        s
            << "insert into agent_config(bind_port, upload_limit, connection_limit) values("
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            NOTICE_LOG("insert one record into agent_config:%s",
                       s.str().c_str());
            break;
        }
    }
...
bool TaskManager::Start() {
    ...
    if (!peer_stat_.Open(configure->peer_stat_file())) {
        WARNING_LOG("Open peer stat file %s failed",
                    configure->peer_stat_file().c_str());
    } else {
        DEBUG_LOG("open stat[%s] file success", configure->peer_stat_file().c_str());
    }

    if (!thrift_tracker_.LoadThriftTrackers(configure->tracker_conf_file())) {
        NOTICE_LOG(
            "Can't load dynamic trackers conf, will try load static tracker conf");
        ...
        if (!thrift_tracker_
            .LoadThriftTrackers(root_path + "/conf/tracker.conf")) {
            FATAL_LOG("load thrift trackers fail\n");
            return false;
        }
    } else {
        DEBUG_LOG("load dynamic tracker conf[%s] success", configure->tracker_conf_file().c_str());
    }
    ...
    if (!control_server_.Start(0777)) {
        ...
        DEBUG_LOG("control server start success");
    }
    ...
    session_.reset(new session(
        ...
        alert::error_notification | alert::peer_notification
            ...
            alert::storage_notification | alert::status_notification
            | alert::performance_warning |
            alert::ip_block_notification | alert::debug_notification));

    error_code ec;
    session_->listen_on(std::make_pair(configure->listen_port(),
    ...
    if (ec) {
        ...
        DEBUG_LOG("listen on port %d success", configure->listen_port());
    }
    {
        ...
        settings.max_metadata_size =
            configure->max_metadata_size() * 1024 * 1024;
        ...
        settings.send_buffer_watermark =
            configure->send_buffer_watermark() * 1024 * 1024;
        settings.send_buffer_low_watermark =
            configure->send_buffer_low_watermark() * 1024;
        settings.send_socket_buffer_size =
            configure->send_socket_buffer_size() * 1024;
        settings.recv_socket_buffer_size =
            configure->recv_socket_buffer_size() * 1024;
    ...
    listen_endpoint_ = libtorrent::tcp::endpoint(
        libtorrent::address::from_string(GetLocalHostIPString(), ec),
        session_->listen_port());
    thrift_tracker_.SetAttribute(session_->id().to_string(), listen_endpoint_.address().to_string(ec));
    session_->add_ex_announce_func(
        boost::bind(&ThriftTracker::ThriftTrackerAnnounce,
                    &thrift_tracker_,
                    _1,
                    _2));
...
void TaskManager::AddTask(const shared_ptr<UnixSocketConnection> &connection,
    ...
    {
        struct ucred cred =
            boost::any_cast<struct ucred>(connection->get_user_argument());
    ...
    if (task_info->has_torrent_path()) {
        ...
        if (size <= 0) {
            WARNING_LOG("torrent file(%s) size not correct.",
                        task_info->torrent_path().c_str());
            ec.assign(bbts::errors::TORRENT_FILE_ERROR, get_error_category());
            return;
        }
        ti.reset(new torrent_info(task_info->torrent_path(),
                                  ec,
                                  0,
                                  task_info->new_name()));
        if (ec) {
        ...
        task_info
            ->set_infohash(libtorrent::to_hex(ti->info_hash().to_string()));
    } else if (task_info->has_data()) {
        const string &data = task_info->data();
        ti.reset(new torrent_info(data.data(),
                                  data.length(),
                                  ec,
                                  0,
                                  task_info->new_name()));
        if (ec) {
            WARNING_LOG("parse torrent data failed, ret(%d): %s.",
                        ec.value(),
                        ec.message().c_str());
            return;
        }
        ...
        string bytes;
        if (task_info->infohash().length() != 40
            || !HexstrToBytes(task_info->infohash(), &bytes)) {
            WARNING_LOG("infohash(%s) not correct.",
                        task_info->infohash().c_str());
            ec.assign(bbts::errors::INFOHASH_NOT_CORRECT, get_error_category());
            return;
        }
        ti.reset(new torrent_info(libtorrent::sha1_hash(bytes),
                                  0,
                                  task_info->new_name()));
        ...
        if (meta) {
            ...
            NOTICE_LOG("get metadata(%s) success",
                       task_info->infohash().c_str());
        } else {
            WARNING_LOG("%s: no metadata to find",
                        task_info->infohash().c_str());
...

    {
        ...
        scoped_lock lock(tasks_lock_);
        infohash_map_t::iterator
            infohash_it = infohash_map_.find(task_info->infohash());
        ...
        if (dup_task) {
            ...
            if (dup_task->get_type() == message::NOCHECK_TASK
                ...
                WARNING_LOG("new task conflict with task(%ld), it will quit.",
                            dup_task->get_id());
...
void
TaskManager::BatchCtrlTasks(const shared_ptr<UnixSocketConnection> &connection,
                            const message::BatchCtrl &message) {
    struct ucred
        cred = boost::any_cast<struct ucred>(connection->get_user_argument());
    message::BatchCtrlRes response;
    tasks_map_t::iterator it;
    if (message.ctrl_all()) {
        scoped_lock lock(tasks_lock_);
...
void TaskManager::TaskSetopt(const shared_ptr<UnixSocketConnection> &connection,
    ...
    shared_ptr<Task> task;
    {
        ...
        if (it == tasks_map_.end()) {
            WARNING_LOG("task(%ld) not found in running tasks",
                        options.taskid());
            ec.assign(bbts::errors::TASK_NOT_FOUND, get_error_category());
            return;
        }
        task = it->second;
        if (!task->CheckCred(boost::any_cast<struct ucred>(connection
                                                               ->get_user_argument()))) {
...
void TaskManager::TaskGetopt(const shared_ptr<UnixSocketConnection> &connection,
    ...
    shared_ptr<Task> task;
    {
        ...
        tasks_map_t::iterator it = tasks_map_.find(options->taskid());
        if (it == tasks_map_.end()) {
            WARNING_LOG("task(%ld) not found in running task",
                        options->taskid());
            ec.assign(bbts::errors::TASK_NOT_FOUND, get_error_category());
            return;
        }
...
void
TaskManager::AgentSetopt(const shared_ptr<UnixSocketConnection> &connection,
    ...
    int upload_rate_limit =
        static_cast<int>(settings.upload_rate_limit / 1024 / 1024);
    std::stringstream sql;
    sql << "update agent_config set connection_limit = "
        << settings.connections_limit
        << ", upload_limit = " << upload_rate_limit;
    bool is_update_success = false;
    for (int i = 0; i < 3; ++i) {
        if (!task_db_->Excute(sql.str())) {
            WARNING_LOG("update agent options in sqlite(%s) failed:%d",
                        sql.str().c_str(),
                        i);
            task_db_->Reconnect();
            continue;
...
void
TaskManager::AgentGetopt(const shared_ptr<UnixSocketConnection> &connection,
...
void
TaskManager::ProcessMessage(const shared_ptr<UnixSocketConnection> &connection,
...
void
TaskManager::OnReadMessage(const shared_ptr<UnixSocketConnection> &connection,
                           const shared_ptr<const std::vector<char> > &data) {
...
void
TaskManager::OnAccept(const boost::shared_ptr<UnixSocketConnection> &connection) {
    ...
    if (!bbts::recv_cred(fd, &cred)) {
        ...
        WriteBaseMessage(connection,
                         error_code(bbts::errors::READ_CRED_FAILED,
                                    get_error_category()));
        return;
    }
...
void TaskManager::SeedingTimerCallback(int taskid, const error_code &ec) {
    if (ec) {
        TRACE_LOG("seeding timer callback error: %s, taskid: %ld",
                  ec.message().c_str(),
                  taskid);
        return;
    }
...
void
TaskManager::RemoveTaskByError(shared_ptr<Task> task, const string &error) {
    task->set_error(error);
    WARNING_LOG("task(%ld) failed, will quit. error: %s.",
                task->get_id(),
                error.c_str());
    ERASE_TASK_FROM_MAP(task);
}
...
void
TaskManager::OnTorrentFinished(const libtorrent::torrent_finished_alert *alert) {
...
void TaskManager::OnListenFailed(const libtorrent::listen_failed_alert *alert) {
    ...
    if (alert->sock_type == libtorrent::listen_failed_alert::tcp
        && ep.address().is_v4()
        && ep.port()
            == LazySingleton<AgentConfigure>::instance()->listen_port()) {
...
void
TaskManager::OnPeerDisconnect(const libtorrent::peer_disconnected_alert *alert) {
...
void TaskManager::ProcessAlert() {
    if (NULL == session_->wait_for_alert(libtorrent::milliseconds(200))) {
        worker_pool_.get_io_service()
            .post(boost::bind(&TaskManager::ProcessAlert, this));
        return;
    }

    deque<alert *> alerts;
    session_->pop_alerts(&alerts);
    for (deque<alert *>::iterator it = alerts.begin(); it != alerts.end();
         ++it) {
        switch ((*it)->type()) {
