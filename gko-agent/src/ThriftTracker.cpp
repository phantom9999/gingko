...
void ThriftTracker::LoadArea(const message::TrackerArea &area) {
    ...
    for (int i = 0; i < area.tracker_size(); ++i) {
        ...
        snprintf(buf,
                 ...
                 tracker.host().c_str(),
                 tracker.port());
        trackers_hashring_[!current_trackers_hashring_index_]
            .AddNode(string(buf));
    }
}
...
void ThriftTracker::GetTrackersByInfohash(const string &infohash,
                                          TrackerVector *trackers) const {
    vector<string> tracker_nodes;
    trackers_hashring_[current_trackers_hashring_index_]
        .GetAllNodes(infohash, &tracker_nodes);
    for (vector<string>::const_iterator it = tracker_nodes.begin();
         it != tracker_nodes.end(); ++it) {
        string ip;
        uint16_t port;
...
void ThriftTracker::ThriftTrackerAnnounce(const torrent_handle &handle,
    ...
    ios_.post(boost::bind(&ThriftTracker::OnAnnounce,
                          this,
                          handle,
                          req->trackers,
                          request));
...
void ThriftTracker::OnAnnounce(const torrent_handle &handle,
    ...
    TRACE_LOG(
        "[%s][seed:%d][state:%d][uploaded:%lld][downloaded:%lld][left:%lld][num_want:%d]",
    ...
    for (TrackerVector::const_iterator it = trackers.begin();
         it != trackers.end(); ++it) {
