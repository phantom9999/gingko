...
void
UnixSocketServer::HandleAccepted(shared_ptr<UnixSocketConnection> connection,
                                 const error_code &ec) {
    if (ec) {
        if (ec
...
void UnixSocketServer::AsyncAccept() {
    ...
    acceptor_.async_accept(connection->get_socket(),
                           ...
                           boost::bind(&UnixSocketServer::HandleAccepted,
                                       this,
                                       connection,
                                       _1));
}
...
bool UnixSocketServer::Serve(mode_t mode) {
    if (CanConnect()) {
        WARNING_LOG("bind address(%s) can connect, can't start serve.",
                    endpoint_.path().c_str());
        return false;
    } else {
        DEBUG_LOG("bind address(%s) can connect", endpoint_.path().c_str());
    }
    ...
    if (ec) {
        ...
        DEBUG_LOG("acceptor open success");
    }

    acceptor_.bind(endpoint_, ec);
    if (ec) {
        WARNING_LOG("bind path(%s) failed: %s",
                    endpoint_.path().c_str(),
                    ec.message().c_str());
        return false;
    }
    ...
    if (ec) {
        WARNING_LOG("set socket(%s) non blocking io failed.",
                    endpoint_.path().c_str());
        return false;
    }
    acceptor_.listen(128, ec);
    if (ec) {
        WARNING_LOG("listen socket(%s) failed: %s",
                    endpoint_.path().c_str(),
                    ec.message().c_str());
        return false;
    }
...
bool UnixSocketServerWithThread::Start(mode_t mode) {
    ...
    boost::thread
        tmp_thread(boost::bind(&UnixSocketServerWithThread::Run, this));
