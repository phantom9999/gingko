...
static void parse_attributes(const map<string, int> &attr_name_map,
    ...
    for (vector<string>::iterator i = v.begin(); i != v.end(); ++i) {
        ...
        bool finded = false;
        for (vector<string>::iterator it = attr_set->begin();
             it != attr_set->end(); ++it) {
            if (*it == *i) {
                finded = true;
                break;
...
static void print_task_status_header(map<string, int> &attr_name_map,
    ...
    for (it = attr_set.begin(); it != attr_set.end(); ++it) {
        switch (attr_name_map[*it]) {
            ...
            case 1:  //status
                fprintf(stdout,
                        "%8s ",
                        message::TaskStatus::status_t_Name(task_status.status())
                            .c_str());
                break;
            ...
            case 4: {  //user
                ...
                int ret = getpwuid_r(task_info.uid(),
                                     &pwd,
                                     buf,
                                     sizeof(buf),
                                     &result);
            ...
            case 5: {  //group
                ...
                char buf[PASSWD_BUF_SIZE] = {0};
                int ret = getgrgid_r(task_info.gid(),
                                     &grp,
                                     buf,
                                     sizeof(buf),
                                     &result);
            ...
            case 14:  //save_path
                fprintf(stdout,
                        "%-10s ",
                        (task_info.save_path() + '/' + task_info.new_name())
                            .c_str());
                break;
...
static int process_mkseed(int argc, char *argv[]) {
    ...
    while (
        (option = getopt_long(argc - 1, &argv[1], short_opt, long_opt, &index))
            != -1) {
        switch (option) {
            case 'r':
            ...
            case 'C':
                if (!parse_cluster_config(optarg,
                                          make_torrent_args.cluster_config)) {
            ...
            case 'p':
                make_torrent_args.full_path =
                    trim_path(complete_full_path(optarg));
                break;

            case 's':
                make_torrent_args.piece_size = atoi(optarg) * 1024;
                if (make_torrent_args.piece_size < 0
                    || make_torrent_args.piece_size > 20 * 1024 * 1024) {
                    fprintf(stderr,
                            "Piece size(%dKB) not valid range(1K ~ 20M).\n",
            ...
            case 2:
                try {
                    make_torrent_args.include_regex
                        .push_back(boost::regex(optarg));
                }
            ...
            case 3:
                try {
                    make_torrent_args.exclude_regex
                        .push_back(boost::regex(optarg));
                }
                catch (boost::bad_expression &e) {
    ...
    if (torrent_file_path.empty()) {
        if (make_torrent_args.full_path.find('*') == string::npos) {
            torrent_file_path = complete_full_path(
                file_name(make_torrent_args.full_path) + ".torrent");
        } else {
            torrent_file_path = complete_full_path(
                file_name(parent_path(make_torrent_args.full_path))
                    + ".torrent");
        }
    }
...
static int process_add(int argc, char *argv[]) {
    ...
    while (
        ...
        switch (option) {
            case 'r':
                task_info
                    ->set_torrent_path(trim_path(complete_full_path(optarg)));
                break;
            ...
            case 'i': {
                ...
                if (info_hash.length() != 40) {
                    fprintf(stderr,
                            "Invalid infohash, must be length 40 for hex\n");
                    exit(1);
                }
            ...
            case 'p':
                if (task_info->new_name().empty()) {
                    ...
                    std::cout << "save_path " <<  task_info->save_path() << std::endl;
                }
                break;
...
static bool
AddIpFilterRule(const string &ip_range, int flags, ip_filter *filter) {
    vector<string> v;
...
static bool
ParseIpFilter(const string &filter_string, int flags, ip_filter *filter) {
    vector<string> ip_range_vector;
    bbts::SliptStringToVector(filter_string, ",", &ip_range_vector);
    for (vector<string>::iterator it = ip_range_vector.begin();
         it != ip_range_vector.end(); ++it) {
...
static int process_download(int argc, char *argv[]) {
    ...
    while (
        ...
        switch (option) {
            ...
            case 'P': {
                ...
                if (start < 1025 || start > 65535 || end < 1025
                    || end > 65535) {
                    fprintf(stderr,
                            "Can't bind port less than 1024 or big than 65535\n");
                    exit(1);
                }
            ...
            case 1:
                params.listen_port_range =
                    std::make_pair(12000 + get_random_num() % 1500, 13999);
                break;
            ...
            case 5:
                ...
                if (params.hang_timeout < 0) {
                    fprintf(stderr,
                            "hang check timeout(%d) less than -1\n",
                            params.hang_timeout);
                    exit(1);
                }
                break;
            ...
            case 19: {
                ...
                if (max_annouce_interval < 10 || max_annouce_interval > 3600) {
                    fprintf(stderr,
                            "the value of max announce interval is not valid\n");
                    exit(1);
                }
            ...
            case 29: {
                ...
                if (params.mem_limit < 100) {
                    fprintf(stderr,
                            "memery limit %dM less than 100M\n",
                            params.mem_limit);
                    exit(1);
                }
                break;
    ...
    if (params.patition_download && params.dynamic_allocate >= 0) {
        fprintf(stderr,
                "Use dynamic allocate mode, but not support partition download currently!\n");
        exit(1);
    }
...
static int process_list(int argc, char *argv[]) {
    ...
    if (is_use_group) {
        ...
        if (client
            .ProcessList(infohash, is_full, is_show_history, is_group_daemon)
            != 0) {
            fprintf(stderr,
                    "List infohash[%s], is_full:%d failed!\n",
                    infohash.c_str(),
                    is_full);
            return 4;
        }
    ...
    for (int i = 0; i < res.status_size(); ++i) {
        ...
        if (task_status.status() != message::TaskStatus::UNKNOW) {
            ...
            fprintf(stdout,
                    "%12ld: %s\n",
                    task_status.task().taskid(),
                    task_status.error().c_str());
            success = false;
        }
    }
    if (success) {
...
static int process_batch_ctrl(int argc,
                              char *argv[],
    ...
    if (is_use_group) {
        ...
        if (client.ProcessControl(infohash, type, ctrl_all) != 0) {
            fprintf(stderr,
                    "control infohash[%s], is_all:%d failed!\n",
                    infohash.c_str(),
                    ctrl_all);
            return 5;
        }
...
static int process_rmfiles(int argc, char *argv[]) {
    ...
    while (
        ...
        switch (option) {
            case 'r':
                delete_args.torrent_path =
                    trim_path(complete_full_path(optarg));
                break;

            case 'p':
                if (delete_args.new_name.empty()) {
                    delete_args.save_path =
                        trim_path(complete_full_path(optarg));
                }
                break;
            ...
            case 'n': {
                string tmp_str = trim_path(complete_full_path(optarg));
                if (!path_slipt(tmp_str,
                                &delete_args.save_path,
                                &delete_args.new_name)) {
                    fprintf(stderr,
                            "newname(%s) not correct!\n",
                            tmp_str.c_str());
                    exit(1);
...
static bool InitConfigure() {
    ...
    configure->set_max_queued_disk_bytes(
        configure->max_queued_disk_bytes() * 1024 * 1024);
    configure->set_send_buffer_watermark(
        configure->send_buffer_watermark() * 1024 * 1024);
    configure->set_send_buffer_low_watermark(
        configure->send_buffer_low_watermark() * 1024);
    configure->set_send_socket_buffer_size(
        configure->send_socket_buffer_size() * 1024);
    configure->set_recv_socket_buffer_size(
        configure->recv_socket_buffer_size() * 1024);
    configure
        ->set_max_metadata_size(configure->max_metadata_size() * 1024 * 1024);
